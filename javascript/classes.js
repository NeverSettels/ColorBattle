...
class PlayerBlock {
  constructor(color = getRandomColor(), initialX, initialY, initialArrY, initialArrX, board, val, img) {
    this.x = initialX
    this.y = initialY
    this.prevX = initialX
    this.prevY = initialY
  ...
  draw() {
    ...
    ctx.drawImage(this.img, this.x, this.y, this.blockSize, this.blockSize)
    ctx.fillStyle = this.color
    ctx.fillRect(this.prevX, this.prevY, this.blockSize, this.blockSize)
  }
  isTouching(bomb) {
    return (
